...
/**
 * The <code>ExcelUtil</code> 与 {@link com.fable.industry.api.export.ExcelCell}搭配使用
 *
...
public class ExcelUtil {
    ...
    public static <T> void exportExcel(Map<String, String> headers, Collection<T> dataset, OutputStream out,
        ...
        write2Sheet(workbook, headers, dataset, pattern);
        try {
            workbook.write(out);
        } catch (IOException e) {
     ...
     *
     * @param <T>
     * @param sheets {@link com.fable.industry.api.export.ExcelSheet}的集合
     * @param out    与输出设备关联的流对象，可以将EXCEL文档导出到本地文件或者网络中
     */
    public static <T> void exportExcel(List<com.fable.industry.api.export.ExcelSheet<T>> sheets, OutputStream out) {
     ...
     * @param pattern 如果有时间数据，设定输出格式。默认为"yyy-MM-dd"
     */
    public static <T> void exportExcel(List<com.fable.industry.api.export.ExcelSheet<T>> sheets, OutputStream out, String pattern) {
     ...
     * @param pattern  日期格式
     */
    private static <T> void write2Sheet(HSSFWorkbook workbook, Map<String, String> headers, Collection<T> dataset,
                                        String pattern) {
        HSSFSheet sheet = workbook.createSheet();
//        sheet.setDefaultColumnWidth(15);
    ...
    private static int setCellValue(HSSFCell cell, Object value, String pattern, int cellNum, Field field, HSSFRow row) {
        ...
        if (value instanceof Integer) {
            ...
            if (field != null) {
                com.fable.industry.api.export.ExcelCell anno = field.getAnnotation(com.fable.industry.api.export.ExcelCell.class);
                if (anno != null) {
     ...
     */
    public static <T> Collection<T> importExcel(int type,Class<T> clazz, InputStream inputStream,
        ...
        try {
            List<com.fable.industry.api.export.ExcelLog> logList = new ArrayList<>();
            // Map<title,index>
                ...
                if (row.getRowNum() == 0) {
                    if (clazz == Map.class) {
                        ...
                        while (cellIterator.hasNext()) {
                            ...
                            // 中文标题找到对应的英文
                            if (type == 1) {
                                titleMap.put(ExcelEnum.getEname(value), index);
                            }else if (type == 0) {
                        ...
                        if (field.getType().isArray()) {
                            ...
                            if (StringUtils.isBlank(errMsg)) {
                                ...
                                if (field.getType().equals(Date.class)
                                    ...
                                    com.fable.industry.api.export.ExcelCell annoCell = field.getAnnotation(com.fable.industry.api.export.ExcelCell.class);
    ...
    private static List<FieldForSortting> sortFieldByAnno(Class<?> clazz) {
        ...
        for (Field field : fieldsArr) {
            com.fable.industry.api.export.ExcelCell ec = field.getAnnotation(ExcelCell.class);
            if (ec == null) {
