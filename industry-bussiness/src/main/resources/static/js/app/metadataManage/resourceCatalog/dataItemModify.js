define(["base", "app/commonApp", "app/metadataManage/resourceCatalog/resourceCatalog"], function (base, common, resourceCatalog) {
    ...
    var setGridOption = function () {
        ...
        gridOption1.drawCallback = function (setting) {
            ...
            //     $(this).find(".signBtn:first").removeClass(".blue").addClass("orange");
            //     if(i>=5){
            //         return false;
            //     }
            // })
            // $("#dataGrid tbody tr").each(function (i,o) {
            //     $(this).find(".signBtn:last").removeClass(".blue").addClass("orange");
            //     if(i>=7){
            //         return false;
            //     }
            // })
    ...
    var setSteps = function (type) {
        steps = base.steps({
            container: $("#ui-steps"),
            currentStep:resourceObj.catalogUEMap.catalogUEState,
            ...
            buttons: [
                ...
                {label: "保存","type":"confirm","cls":"btn btn-info blue","icon":"fa fa-check","callback":function(){
                    base.ajax({
                        ...
                        type: "post",
                        params: {"catalogUEState": 4, "resciState": 0, "id": $("#rowId").attr("rid")},
                        success: function (d) {
                            if(d.success){
                                base.page.back({
                                    callback:function(){
                ...
                {label: "保存并提交","type":"confirm","cls":"btn btn-info blue","icon":"fa fa-check","callback":function(){
                    ...
                    base.ajax({
                        ...
                        type: "post",
                        async:false,
                        params: {"resciState": 1, "id": $("#rowId").attr("rid")},
                        success: function (d) {
                            base.page.back({
                                callback:function(){
                                    if(resourceCatalog){
        ...
        steps.forward(function () {
            ...
            switch (step) {
                ...
                case 1:
                    ...
                    if(resourceObj.catalogUEMap.resciCode != $("#form2 input[name='resciCode']").val()){
                        base.ajax({
                ...
                case 2:
                    ...
                    base.ajax({
                        ...
                        type: "post",
                        params: {"resciCode": $("#form2 input[name='resciCode']").val(), "oid": params.oid},
                        async: false,
                        success: function (d) {
                            if (d.success) {
                                common.addLoadBg();
                    ...
                    if(resourceObj.catalogUEMap.catalogUEState<3){
                        setGridOption();
                        drawGrid();
                    }
                    return pass;
                ...
                case 3:
                    ...
                    var isQ = false;
                    var isT = false;
                    params.catalogueId = $("#rowId").attr("rid");
                    $("#dataGrid tbody tr").each(function (i, o) {
    ...
    var loading = function () {
        ...
        $("#imageUp input[type='button']").on("click",function(){
            $("#form2 #file").remove();
            $("#form2").append("<input type='file' id='file' name='file' accept = '.png,.jpg,.pdf,image/png,image/jpg,image/pdf' style='display: none'>")
            $("#file").trigger("click");
        })
        ...
        $("#form2 select[name='resciLevel']").val(resourceObj.catalogUEMap.resciLevelId==null?0:resourceObj.catalogUEMap.resciLevelId);
        $("#form2 #imageUp div").html(resourceObj.catalogUEMap.image);
        ...
        $.each(resourceObj.dataElmentList,function (i,o) {
            ...
            var qTitle = o.isQuery == 1?"去除查询条件":"添加为查询条件";
            var tTitle = o.isTableDisplay == 1?"去除列表展示项":"添加列表展示项";
            var html =  "<tr><td>"+o.dataName+"</td>"+
                ...
                "<button rid=''  class='btn btn-link blue shiftUp' type='role' name='上移' title='上移' onclick='return false'><i class='fa fa-arrow-up'></i></button>" +
                "<button rid='' class='btn btn-link blue shiftDown' type='role' name='下移' title='下移' onclick='return false'><i class='fa fa-arrow-down'></i></button>" +
                "<button rid=''  class='btn btn-link "+isQuery+" signBtn' type='role' name='' title='"+qTitle+"' onclick='return false'><i class='fa fa-search'></i></button>" +
                "<button rid='' class='btn btn-link "+isTableDisplay+" signBtn' type='role' name='' title='"+tTitle+"' onclick='return false'><i class='fa fa-align-justify'></i></button>" +
                "<input type='hidden' class='hiddenId' rid='" + o.dataElementId + "'>" +
                "<input type='hidden' class='hiddenDataTypeId' rid='" + o.dataTypeId + "'>"+
                "<input type='hidden' class='establishType' rid='"+o.establishType+"'>"+
                "<input type='hidden' class='dictionaryId' rid='"+o.dictionarId+"'>"+
                "<input type='hidden' class='constraint' rid='"+o.constraint+"'>"
            "</td></tr>"
            $("#dataGrid tbody").append(html);
        })
        base.form.init(resourceObj.catalogUEMap,$("#form5"));
        $("#form5 img").attr("src",$.base+"/"+resourceObj.catalogUEMap.imagePath);
        $.each(resourceObj.dataElmentList,function (i,o) {
            var i = i+1
            var html = "<tr>"+
                "<td>"+i+"</td>"+
