...
define(["jquery", "bootstrap"], function() {
	...
	base.ajax = function(option, isConnect) {
		...
		if(option) {
			...
			self.beforeSendTip = option.beforeSendTip?option.beforeSendTip:null;
			self.successTip = option.successTip?option.successTip:null;
			self.errorTip = option.errorTip?option.errorTip:null;
			...
			self.connect = function() {
				var ajaxOption = {
					...
					complete: function(XMLHttpRequest, textStatus) {
						if(self.complete) {
							self.complete(XMLHttpRequest, textStatus);
	...
	/**loading(读取中)组件**/
	base.loading = function(container,color) {
		color = color?color:"#aaa";
		$(container).html("<div class='ui-loading' style='display:table;width:100%;height:100%;text-align:center;'>" +
			"<div style='display: table-cell;vertical-align: middle;'><i class='fa fa-spinner fa-pulse fa-3x fa-fw' style='font-size:24px;color:"+color+"'></i><span style='margin-right:5px;color:"+color+"'>加载中...</span></div></div>");
	};
	...
	base.progress = function(option) {
		...
		self.bg = option.bg ? option.bg : "#111";
		self.radius = option.radius ? option.radius : 5;
		self.label = null;
		...
		self.setProgress = function() {
			...
			$(self.progress).css("background", self.bg);
			...
			for(var key in role) {
				...
				switch(key) {
						...
						if(role[key]) {
							...
							switch($(obj)[0].tagName.toLowerCase()) {
								case "select":
									...
									if(!val || val == "*") {
										errorText = role.errorText?role.errorText:"必选项";
					...
					case "number": //是否是数字
						...
						if(isNaN(Number(val))) {
							errorText = role.errorText?role.errorText:"必须是" + ds;
							hasError = self.error(role, errorText, obj);
						} else {
								...
								switch((typeof(role[key])).toLowerCase()) {
									case "number":
										...
										if(Number(val) > role[key]) {
											errorText = role.errorText?role.errorText:"数字不能大于" + role[key];
											hasError = self.error(role, errorText, obj);
										}
										break;
									...
									default:
										if(jQuery.isArray(role[key]) && role[key].length >= 2) {
											if(val < role[key][0]) {
												errorText = role.errorText?role.errorText:"不能小于" + role[key][0];
												hasError = self.error(role, errorText, obj);
											} else if(val > role[key][1]) {
												errorText = role.errorText?role.errorText:"不能大于" + role[key][1];
												hasError = self.error(role, errorText, obj);
											}
										}
										break;
					...
					case "length": //字符串长度
						...
						if(jQuery.isArray(role[key])) {
							if(role[key].length==1){
								if(base.getByteLen(val) < role[key][0]) {
									errorText = role.errorText?role.errorText:"不能小于" + role[key][0]+ "个字符";
									hasError = self.error(role, errorText, obj);
								}
								
							}else if(role[key].length==2){
								if(base.getByteLen(val) < role[key][0]) {
									errorText = role.errorText?role.errorText:"不能小于" + role[key][0]+ "个字符";
									hasError = self.error(role, errorText, obj);
									return;
								}else if(base.getByteLen(val) > role[key][1]){
									errorText = role.errorText?role.errorText:"不能超过" + role[key][1] + "个字符";
									hasError = self.error(role, errorText, obj);
									return;
								}
							}
							...
							if(base.getByteLen(val) > role[key]) {
								errorText = role.errorText?role.errorText:"不能超过" + role[key] + "个字符";
								hasError = self.error(role, errorText, obj);
							}
						}
					...
					case "money": //货币 
						...
						if(isNaN(val)) {
							errorText = role.errorText?role.errorText:"格式不正确";
							hasError = self.error(role, errorText, obj);
						} else {
							val = Number(val);
							if(!ex.test(val)) {
					...
					case "filter": //只允许字母、数字、下划线
						...
						if(!ex.test(val)) {
							...
							errorText = role.errorText?role.errorText:"只允许字母、数字、下划线";
					...
					case "filterCN":
						...
						if(!ex.test(val)) {
							...
							errorText = role.errorText?role.errorText:"只允许中英文、数字、下划线";
					...
					case "first":
						...
						if(val.slice(0, l) != fs) {
							errorText = role.errorText?role.errorText:"必须以" + fs + "开头";
							hasError = self.error(role, errorText, obj);
						}
					...
					case "website":
						...
						if(!ex.test(val)) {
							errorText = role.errorText?role.errorText:"网络地址不正确";
							hasError = self.error(role, errorText, obj);
						}
					...
					case "custom":
						...
						if(role.custom){
							ex = eval(role.custom);
							errorText = role.errorText?role.errorText:"内容不正确";
							if(!val || val == "") {
								return;
							}
							if(!ex.test(val)) {
	...
	base.form.treeSelect = function(option) {
		...
		self.height = option.maxHeight?option.maxHeight:180;
		self.width = option.width?option.width:null;
		self.checkboxType = option.checkboxType?option.checkboxType:null;
		self.setting = {};
		if(self.clickCallback) {
			self.clickEvent = function(event, treeId, treeNode) {
				...
				if(self.clickCallback) {
					eventReturn = self.clickCallback(event, treeId, treeNode);
                    debugger
				}
				if(eventReturn!="break"){
		...
		if(self.checkboxType){
			self.setting.check = {
				enable:true,
				chkStyle:"checkbox",
				chkboxType: self.checkboxType
			}
		}
		...
		self.createTree = function() {
			...
			$(self.treeContainer).css("width", self.width?self.width:$(self.container).outerWidth());
	...
	base.datatables = function(option) {
		...
		self.idDef = option.idDef?option.idDef:"id";
		if(self.nodataIcon){
								...
								if(self.selectType=="checkbox"||self.selectType=="radio"){
									self.tempOption.columnDefs.push(
										{"render":function(data,type,row,meta){
		                					...
		                					return "<input  type='"+self.selectType+"' name='cb' value='"+row[self.idDef]+"' "+ats+"/>";
		              					}, 
		               					"targets":0 
		          						}
									);
		...
		self.jumpPage = function(pageNumber){
			self.grid.page(pageNumber).draw(false);
		};
		self.refresh = function(){
			self.jumpPage(self.getPageNumber())
		};
		self.getPageNumber = function(){
			return self.grid.page();
		};
		self.create();
		return self;
	};
	...
	base.requestTip = function(option) {
		...
		self.create = function() {
			...
			$(self.container).addClass("ui-requestTip");
		...
		self.setBody = function() {
			...
			self.body.createCarouselItem = function(data, index) {
				...
				if(data.contentToggle) {
					...
					$(item).find("*").on("keydown", function(e) { 
		...
		self.setFooter = function() {
			...
			self.footer.check = function(obj) {
				...
				if(self.currentStep == 0) { //第一步
					...
					$(self.footer.element).find(".prev").addClass("disabled");
					$(self.footer.element).find(".prev").hide();
					$(self.footer.element).find(".next").removeClass("disabled");
					$(self.footer.element).find(".next").show();
					...
					$(self.footer.element).find(".prev").removeClass("disabled");
					$(self.footer.element).find(".prev").show();
					$(self.footer.element).find(".next").addClass("disabled");
					$(self.footer.element).find(".next").hide();
	...
	/**获取当前日期**/
	base.getNowDate = function(cn,autoZero) {
		var myDate = null;
		if(cn){
			if(autoZero){
				myDate = (new Date()).Format("yyyy年M月d日");
			}else{
				myDate = (new Date()).Format("yyyy年MM月dd日");
			}
		}else{
			if(autoZero){
				myDate = (new Date()).Format("yyyy-M-d");
			}else{
				myDate = (new Date()).Format("yyyy-MM-dd");
			}
		}
		return myDate;
	};
	...
	/**获取当前时间**/
	base.getNowTime = function(cn,autoZero) {
		var myDate = null;
		if(cn){
			if(autoZero){
				myDate = (new Date()).Format("yyyy年M月d日 h:m:s");
			}else{
				myDate = (new Date()).Format("yyyy年MM月dd日 hh:mm:ss");
			}
		}else{
			if(autoZero){
				myDate = (new Date()).Format("yyyy-M-d h:m:s");
			}else{
	...
	base.pillTabs = function(option) {
		...
		self.tabContainer = null;
		self.tab = null;
		self.leftArrowHandle = null;
		self.rightArrowHandle = null;
		self.activeIndex = option.activeIndex?option.activeIndex:0;
		self.size = option.size?option.size:6;
		//self.itemWidth = option.itemWidth?option.itemWidth:120;
		
		self.itemHeight = option.itemHeight?option.itemHeight:30;
		self.currentPageNumber = (self.activeIndex-self.activeIndex%self.size)/self.size;
		self.pageNumberCount = (self.data.length - self.data.length%self.size)/self.size;
		self.level = 0;
		self.clickEvent = option.clickEvent?option.clickEvent:null;
		self.margin =2;
		self.isPageActive = option.isPageActive==false?false:true;
		self.maxWidth = 150;
		self.itemWidth = ($(self.container).width()-80-self.margin*self.size)/self.size;
		if(self.itemWidth>self.maxWidth){
		...
		self.createTabBar = function(){
			self.tabBar = document.createElement("div");
			$(self.tabBar).addClass("ui-tabBar");
			$(self.container).append(self.tabBar);
		};
		self.createTabContainer = function(){
			self.tabContainer = document.createElement("div");
			$(self.tabContainer).addClass("ui-tabContainer");
			$(self.tabBar).append(self.tabContainer);
		};
		self.createTab = function(){
			self.tab = document.createElement("ul");
			$(self.tab).addClass("ui-pillTabs");
			base.setProperty(self.tab,option);
			$(self.tabContainer).append(self.tab);
			$(self.data).each(function(i, o) {
				self.createTabItem(o,i);
			});
		};
		
		self.createHandle = function(){
			if(self.data.length>self.size){
				$(self.tabContainer).css("width",(self.itemWidth+self.margin)*self.size);
				$(self.tabContainer).css("overflow","hidden");
				$(self.tabContainer).css("float","left");
				self.leftArrowHandle = document.createElement("div");
				$(self.leftArrowHandle).html("<i  style='font-size:18px' class='fa fa-angle-left'></i>");
				$(self.tabBar).prepend(self.leftArrowHandle);
				self.rightArrowHandle = document.createElement("div");
				$(self.leftArrowHandle).addClass("ui-pillTabs-handle");
				$(self.rightArrowHandle).addClass("ui-pillTabs-handle");
				$(self.leftArrowHandle).css("cursor","pointer");
				$(self.rightArrowHandle).css("cursor","pointer");
				$(self.rightArrowHandle).css("padding",0);
				$(self.rightArrowHandle).html("<i style='font-size:18px' class='fa fa-angle-right'></i>");
				$(self.tabBar).append(self.rightArrowHandle);
				$(self.tab).css("width",self.data.length*(self.itemWidth+self.margin)+"px");
				
				$(self.leftArrowHandle).css("text-align","center");
				$(self.rightArrowHandle).css("text-align","center");
				$(self.leftArrowHandle).css("float","left");
				$(self.rightArrowHandle).css("float","left");
				$(self.leftArrowHandle).css("width","40px");
				$(self.rightArrowHandle).css("width","40px");
				//$(self.leftArrowHandle).css("height",self.itemHeight);
				//$(self.leftArrowHandle).css("line-height",self.itemHeight+"px");
				//$(self.rightArrowHandle).css("height",self.itemHeight);
				//$(self.rightArrowHandle).css("line-height",self.itemHeight+"px");
				$(self.leftArrowHandle).on("click",function(){
					if(!self.isPageActive){
						...
						if(self.currentPageNumber>0){
							self.setPageActive(self.currentPageNumber--);
						}
				...
				$(self.rightArrowHandle).on("click",function(){
					if(!self.isPageActive){
						...
						if(self.currentPageNumber<self.pageNumberCount){
		...
		self.setPageActive = function(){
			var l = (self.itemWidth+self.margin)*self.size*self.currentPageNumber;
			$(self.tab).animate({"marginLeft":-l});
			
		};
		self.setItemActive= function(){
			$(self.tab).children("li").removeClass("active");
			var activeObj = $(self.tab).children("li:eq("+self.activeIndex+")");
			$(activeObj).addClass("active");
			var activeData = self.data[self.activeIndex];
			if(activeData.clickEvent){
				activeData.clickEvent(activeObj, activeData, self);
			}else if(self.clickEvent) {
				self.clickEvent(activeObj, activeData, self);
			}
			if(!self.isPageActive){
				var level = (self.activeIndex-self.activeIndex%self.size)/self.size;
				if(level!=self.level){
					var l = level*(self.itemWidth+self.margin)*self.size;
					$(self.tab).animate({"marginLeft":-l});
					self.level = level;
				}
			}
		...
		self.createTabItem = function(data,index) {
			if(!data){return;}
			var tabItem = document.createElement("li");
			$(tabItem).attr("title",data.name);
			$(tabItem).css("width",self.itemWidth);
			//$(tabItem).css("height",self.itemHeight);
			//$(tabItem).css("line-height",self.itemHeight+'px');
			$(tabItem).css("margin-right",self.margin+"px");
			if(data.imageIcon) {
			...
			if(data.value){
				$(tabItem).attr("val",data.value);
			}
			
			$(self.tab).append(tabItem);
	...
	base.accordion = function(option) {
		...
		self.accordion = null;
		self.data = option.data ? option.data : null;
		self.id = option.id ? option.id : "accordion" + base.getRandom(1000, 9999);
	...
	/**禁止tab键**/
	base.handleTabKey = function(container){
   ...
   /**弹幕插件**/
	base.barrage = function(option){
		var self = {};
		self.container = option.container?option.container:null;
		self.colors = option.colors?option.colors:["#00ffea"];
		self.images = option.images?option.images:null;
		self.data = option.data?option.data:null;
		self.drawCallback = option.drawCallback?option.drawCallback:null;
		self.timeRange = option.timeRange?option.timeRange:[5,10];
		self.fontRange = option.fontRange?option.fontRange:[12,16];
		self.repeat = option.repeat==false?false:true;
		self.repeatNum = option.repeatNum?option.repeatNum:2;
		self.width = option.width?option.width:140;
		self.timeGroup = [];
		self.init = function(){
			...
			this.run = function(){
				if(self.data&&self.data.length>0){
					for(var i=0,j=self.data.length;i<j;i++){
						for(var i1=0;i1<self.repeatNum;i1++){
		...
		self.createItem = function(text){
			var text = text?text:"";
			var item = document.createElement("div");
			$(item).css("position","absolute");
			$(item).css("width",self.width);
			var size = base.getRandom(self.fontRange[0],self.fontRange[1]);
		...
		self.animate = function(item){
			$(item).animate({left:-$(item).width()},base.getRandom(self.timeRange[0],self.timeRange[1])*1000,function(){
				if(self.repeat){
					self.setPosition(item);
					self.animate(item);
				}
		...
		self.setTheme = function(item){
			if(self.colors&&self.colors.length>0){
				var rdn = base.getRandom(0,self.colors.length-1);
				color = self.colors[rdn]?self.colors[rdn]:self.colors[0];
				$(item).css("color",color);
				if(self.images&&self.images.length>0){
					image = self.images[rdn]?self.images[rdn]:self.images[0];
					$(item).css("background","url('"+image+"')");
					$(item).css("background-repeat","repeat-y");
				}
			}
		};
		self.setPosition = function(item){
			var h = $(self.container).height();
			var t = base.getRandom(0,h-20);
			var w  = $(self.container).width();
			$(item).css("left",w+base.getRandom(0,60));
			$(item).css("top",t);
			self.setTheme(item);
		};
		self.empty = function(){
			$(self.container).html("");
			for(var i=0,j=self.timeGroup.length;i<j;i++){
				window.clearInterval(self.timeGroup[i]);
			}
		};
	...
	/**滚动翻牌组件**/
	base.rollBrand = function(option){
		var self = {};
		self.container = option.container?option.container:null;
		self.autoAnimate = option.autoAnimate==false?false:true;
		self.data = option.data?option.data:null;
		self.element = null;
		self.hasTheme = option.hasTheme==true?true:false;
		self.gap = option.gap?option.gap:27;
		self.bg = option.bg?option.bg:"#004c69";
		self.color = option.color?option.color:"#97e5ff";
		self.radius = option.radius?option.radius:0;
		self.size = option.size?option.size:5;
		self.zIndex = option.zIndex?option.zIndex:1000;
		self.brandItemNum = 0;
		self.activeNumber = option.activeNumber?option.activeNumber:0;
		self.activeArrowUrl = option.activeArrowUrl?option.activeArrowUrl:null;
		self.arrow = null;
		self.arrowOffset = option.arrowOffset?option.arrowOffset:[2,-4];
		self.rollCallback = option.rollCallback?option.rollCallback:null;
		self.time = option.time?option.time:2000;
		self.create = function(){
			if(!self.container){return;}
		...
		self.createBrand = function(){
			self.element = document.createElement("ul");
			$(self.element).addClass("ui-rollBrand");
			$(self.container).append(self.element);
			if(self.data.length%self.size>0){
				self.brandItemNum = (self.data.length-(self.data.length%self.size))/self.size+1;
			}else{
				self.brandItemNum = (self.data.length-(self.data.length%self.size))/self.size;
			}
			for(var i=0;i<self.brandItemNum;i++){
				self.createBrandItem(i);
			}
			if(self.activeArrowUrl){
				self.createArrow();
			}
			
			self.animate();
			if(self.rollCallback){
				self.rollCallback(0,self);
			}
		};
		self.createArrow = function(){
			self.arrow = document.createElement("img");
			$(self.arrow).attr("src",self.activeArrowUrl);
			$(self.arrow).css("position","absolute");
			$(self.arrow).css("z-index",self.zIndex);
			self.setArrowPosition(0);
			$("body").append(self.arrow);
		};
		self.setArrowPosition = function(index){
			var ao = $(self.element).find(".ui-brandItemChild:eq("+index+")");
			var t =  $(ao).offset().top+self.arrowOffset[1];
			var l =  $(ao).offset().left+$(ao).width()+self.arrowOffset[0];
			$(self.arrow).css("left",l);
			$(self.arrow).css("top",t);
			//$(self.arrow).animate({"top":t,"left":l});
		};
		self.createBrandItem = function(number){
			var item = document.createElement("li");
			if(number==self.activeNumber){
				$(item).addClass("active");
			}
			$(self.element).append(item);
			var data = null;
			
			if(number==self.brandItemNum-1){
				data = self.data.slice(number*self.size,self.data.length);
			}else{
				data = self.data.slice(number*self.size,(number+1)*self.size);
			}
			
			$(data).each(function(i,o){
				if(i==0){
					self.createBrandItemChild(o,item,true);
				}else{
					self.createBrandItemChild(o,item);
				}
				
			});
			
		};
		self.createBrandItemChild = function(data,item,isActive){
			var child = document.createElement("div");
			$(child).addClass("ui-brandItemChild");
			$(child).css("text-align","center");
			if(isActive){
				$(child).addClass("active");
			}
			$(child).text(data.name);
			$(child).attr("val",data.value);
			$(item).append(child);
		};
		self.animate = function(){
			var i = 0;
			var activeBrandObj = null;
			...
			window.setInterval(function(){
				...
				$(self.element).find(".ui-brandItemChild").removeClass("active");
				activeChildObj = $(self.element).find(".ui-brandItemChild:eq("+i+")");
				$(activeChildObj).addClass("active");
				if(i%self.size==0){
					$(self.element).children("li").removeClass("active");
					$(activeChildObj).parent().addClass("active");
				}
				self.setArrowPosition(i);
				if(self.rollCallback){
					self.rollCallback(i,self);
				}
			},self.time);
	...
	/**数字累加减动画**/
	base.animate.digital = function(option){
		var self = {};
		self.container = option.container?option.container:null;
		self.value = option.value?option.value:null;
		self.type = option.type?option.type:"asc";
		self.speed = option.speed?option.speed:2;
		self.create = function(){
			if(!self.container){return;}
			if(!self.value||self.value<0){return;}
			switch(self.type){
				case "asc":
					$(self.container).html("0");
				break;
				
				case "desc":
					$(self.container).html(self.value);
				break;
			}
		...
		self.animate = function(){
			...
			switch(self.type){
				case "asc":
					...
					t = window.setInterval(function(){
						if(i>=self.value){
							...
							i = i+base.getRandom(0,self.value/(self.speed*10));
						}
						$(self.container).html(i);
					},self.speed*10);
				break;
				...
				case "desc":
					...
					t = window.setInterval(function(){
						i = i-base.getRandom(0,self.value/(self.speed*10));
						if(i<0){i=0;window.clearInterval(t);}
						$(self.container).html(i);
					},self.speed*10);
				break;
			}
	...
	/**蜂窝效果组件**/
	base.beehive = function(option){
		var self = {};
		self.container = option.container?option.container:null;
		self.data = option.data?option.data:null;
		self.size = option.size?option.size:50;
		self.beehive = null;
		self.clickEvent = null;
		self.create = function(){
			self.createBeehive();
			$(self.data).each(function(i,o){
				self.createBeehiveItem(i,o);
			});
		};
		self.createBeehive = function(){
			self.beehive = document.createElement("ui");
			$(self.beehive).addClass("ui-beehive");
			$(self.container).append(self.beehive);
		};
		self.createBeehiveItem = function(index,data){
			var item = document.createElement("li");
			$(item).addClass("ui-beehive-item");
			var content = "<a class='ui-beehive-item-in' val='"+data.value+"' index='"+index+"' href='#'><img/><div><div class='ui-beehive-item-text'><div>"+data.name+"</div></div></div></a>";
			$(item).html(content);
			$(self.beehive).append(item);
			if(self.clickEvent){
				$(item).find("a").on("click",function(){
					self.clickEvent(this,data);
				});
			}
	...
	/**校验2个ip地址是否在同一个域**/
	base.checkDomain = function(ip1,ip2,isCompare){
		var a = ip1.split(".");
		var b = ip2.split(".");
		var isCompare = isCompare==false?false:true;
		var isDomain = true;
